...
void reconnect() {
  while (!client.connected()) {
    ...
    if (globalClientId == "") {
      globalClientId = "ESP32S3-FocusTrack-" + WiFi.macAddress();
    }
    
    // Conectar usando usuario y contraseña
    const char* mqtt_user = "juanito";
    const char* mqtt_pass = "hola123";
...
void setup() {
  ...
  // Ignorar la validación del certificado (útil para certificados autofirmados de prueba)
